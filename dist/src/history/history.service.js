...
let HistoryService = class HistoryService {
    ...
    async rateAndCommentHistoryForPassengerById(id, passengerId, updateHistoryDto) {
    ...
    async rateAndCommentHistoryForRidderById(id, ridderId, updateHistoryDto) {
        return await this.db.update(history_schema_1.HistoryTable).set({
            starRatingByPassenger: updateHistoryDto.starRating,
            commentByPassenger: updateHistoryDto.comment,
            updatedAt: new Date(),
    ...
    async delinkHistoryByPassengerId(id, passengerId) {
        const responseOfUpdatingHistory = await this.db.update(history_schema_1.HistoryTable).set({
            passengerId: null,
        }).where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(history_schema_1.HistoryTable.id, id), (0, drizzle_orm_1.eq)(history_schema_1.HistoryTable.passengerId, passengerId)))
            .returning({
    ...
    async delinkHistoryByRidderId(id, ridderId) {
        const responseOfUpdatingHistory = await this.db.update(history_schema_1.HistoryTable).set({
            ridderId: null,
        }).where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(history_schema_1.HistoryTable.id, id), (0, drizzle_orm_1.eq)(history_schema_1.HistoryTable.ridderId, ridderId)))
            .returning({
            passengerId: history_schema_1.HistoryTable.passengerId,
            ridderId: history_schema_1.HistoryTable.ridderId,
        });
