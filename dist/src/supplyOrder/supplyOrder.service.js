...
let SupplyOrderService = class SupplyOrderService {
    ...
    async searchBetterFirstSupplyOrders(creatorName = undefined, limit, offset, isAutoAccept, getBetterSupplyOrderDto, searchPriorities) {
        ...
        if (getBetterSupplyOrderDto.startAfter || getBetterSupplyOrderDto.endedAt) {
            timeQuery = (0, drizzle_orm_1.sql) `(
              ${getBetterSupplyOrderDto.startAfter ?
                (0, drizzle_orm_1.sql) `ABS(EXTRACT(EPOCH FROM (${supplyOrder_schema_1.SupplyOrderTable.startAfter} - ${getBetterSupplyOrderDto.startAfter})))` :
                (0, drizzle_orm_1.sql) ``}
              ${getBetterSupplyOrderDto.startAfter && getBetterSupplyOrderDto.endedAt ? (0, drizzle_orm_1.sql) ` + ` : (0, drizzle_orm_1.sql) ``}
              ${getBetterSupplyOrderDto.endedAt ?
                (0, drizzle_orm_1.sql) `ABS(EXTRACT(EPOCH FROM (${supplyOrder_schema_1.SupplyOrderTable.endedAt} - ${getBetterSupplyOrderDto.endedAt})))` :
                (0, drizzle_orm_1.sql) ``}
            ) ASC`;
        }
        ...
        updatedAtQuery = (0, drizzle_orm_1.sql) `${supplyOrder_schema_1.SupplyOrderTable.updatedAt} DESC`;
        aboutToStartQuery = (0, drizzle_orm_1.sql) `${supplyOrder_schema_1.SupplyOrderTable.startAfter} ASC`;
        const sortMap = {
            'T': timeQuery,
            'R': routeQuery,
            'S': startQuery,
