...
const AnimatedInputMessage = (props: AnimatedInputMessageProps) => {
    ...
    const [inputValues, setInputValues] = useState<string[]>(
        props.inputAttributes.map((attr) => attr.defaultValue || '')
    );
    const [countdowns, setCountdowns] = useState<number[]>(
        Array(props.inputAttributes.length).fill(0)
    );
    ...
    const handleInputSideButtonOnClick = (index: number) => {
        ...
        if (props.inputAttributes[index].inputSideButton) {
            ...
            <Animated.View style={styles.messageBox} onTouchEnd={(e) => e.stopPropagation()}>
                            ...
                            {attributes.inputSideButton && 
                                ...
                                >
                                    <Animated.Text style={styles.inputSideButtonTitle}>
                                        {countdowns[index] > 0
                                        ? `${countdowns[index]}秒`
                                        : attributes.inputSideButton.title}
                                    </Animated.Text>
                                </Pressable>
                            }
                        </Animated.View>
                ...
                <Animated.View style={styles.buttonContainer}>
                    <TouchableOpacity
                        ...
                        onPress={handleLeftOptionClick}
                    >
                        <Animated.Text style={styles.leftButtonText}>
                            {props.leftOptionTitle}
                        </Animated.Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                        <Animated.Text style={styles.rightButtonText}>
                            {props.rightOptionTitle}
                        </Animated.Text>
                    </TouchableOpacity>
                </Animated.View>
            </Animated.View>
