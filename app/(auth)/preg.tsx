  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await axios.post(
        'https://moto-share-jeffs-projects-95ef1060.vercel.app/auth/signUpPassenger',
        {
          email: username,
          password: password
        },
      ...
      if (response && response.data[0].email === username) {
        dispatch(setUser({ id: response.data[0].id, username: response.data[0].userName, role: 1 }));
