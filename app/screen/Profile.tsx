...
const Profile = () => {
  ...
  const getUserAuths = async (token: string) => {
    if (token.length !== 0 && user.auth === null) {
      try {
        const response = await api.get(user.role === "Passenger" ? "/passengerAuth/getMyAuth" : "/ridderAuth/getMyAuth", {
          headers: {
  ...
  const getUserBank = async (token: string) => {
    if (token.length !== 0 && user.auth === null) {
      try {
        const response = await api.get(user.role === "Passenger" ? "/passengerBank/getMyBalance" : "/ridderBank/getMyBalance", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleLogoutButtonOnClick = async () => {
    ...
    try {
      if (token && token.length !== 0) {
        await api.patch(user.role === "Passenger" ? "/passenger/resetAccessTokenToLogout" : "/ridder/resetAccessTokenToLogout", null, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleDeleteMeButtonOnClick = async (inputValues: string[]) => {
    ...
    try {
      if (token && token.length !== 0) {
        await api.delete(user.role === "Passenger" ? "/passenger/deleteMe" : "/ridder/deleteMe", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          data: {
  ...
  const listData = [
    { id: "1", icon: require("../../assets/images/checklist.png"), label: t("My Create Order"), callback: () => navigation.navigate("mycreateorder" as never) },
    { id: "2", icon: require("../../assets/images/recurring-order.png"), label: t("Recurring Orders"), callback: () => navigation.navigate("porder" as never) },
    {
      id: "3", 
    ...
    { id: "7", icon: require("../../assets/images/transfer-money.png"), label: t("Recharge Balance"), callback: () => navigation.navigate("payment" as never) },
    { id: "8", icon: require('../../assets/images/setting.png'), label: t("System Settings"), callback: () => navigation.navigate("settings" as never) },
    { id: "9", icon: require("../../assets/images/reset-password.png"), label: t("Reset email and password"), callback: () => navigation.navigate("resetemailpassword" as never) },
    { id: "10", icon: require("../../assets/images/support.png"), label: t("feedback"), callback: () => navigation.navigate("report" as never) },
  ];

  return (isLoading && !isAbsoluteLoading) || !styles || !theme ? (
    <LoadingWrapper />
  ) : (
    <View
      style={{
      ...
      <View style={styles.container}>
        ...
        <FlashList
          ...
          renderItem={({ item }: any) => (
            <SettingButton 
              ...
              {...(item.badge > 0 && { badgeCount: item.badge })}
              theme={theme}
        ...
        <View style={styles.bottomButtonContainer}>
          <Pressable
            ...
            style={({ pressed }) => [styles.logoutButton, pressed && { backgroundColor: "rgba(0, 0, 0, 0.1)" }]}
          >
            <Animated.View
              style={{
                ...styles.logoutButton,
          ...
          <Pressable
            ...
            style={({ pressed }) => [styles.deleteAccountButton, pressed && { backgroundColor: "rgba(225, 0, 0, 1)" }]}
          >
            <Animated.View
              style={{
          ...
          {checkDeleteMeMessageDisplay && (
            <AnimatedInputMessage
              ...
              theme={theme}
              inputAttributes={[{ placeholder: t("password"), isSecureText: true }]}
              leftOptionTitle={t("confirm")}
