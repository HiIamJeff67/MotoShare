        ...
        const responseOfCreatingPurchaseOrders = await tx.insert(PurchaseOrderTable).values(
          responseOfSelectingPeriodicPurchaseOrders.map((content) => {
            return {
              ...
              startCord: sql`ST_SetSRID(ST_MakePoint(${content.startCord.x}, ${content.startCord.y}), 4326)`, 
              endCord: sql`ST_SetSRID(ST_MakePoint(${content.endCord.x}, ${content.endCord.y}), 4326)`, 
